...
int main(int argc, char* argv[])
{
    ...
    int search_index = 0 ; //start searching argv at the first global data symbol

    while( search_index < argc)
    {
        search_index = search_index+2 ;

        if(argv[search_index] == "in"){ in_offset = (uint16_t) hex_string_to_uint32(argv[search_index+1]) ; continue ; }
        if(argv[search_index] == "out"){ out_offset = (uint16_t) hex_string_to_uint32(argv[search_index+1]) ; continue ; }
        if(argv[search_index] == "counter"){ counter_offset = (uint16_t) hex_string_to_uint32(argv[search_index+1]) ; continue ; }                     
        if(argv[search_index] == "buffer_size"){ buffer_size_offset = (uint16_t) hex_string_to_uint32(argv[search_index+1]) ; continue ; } 
        if(argv[search_index] == "buffer"){ buffer_offset = (uint16_t) hex_string_to_uint32(argv[search_index+1]) ; continue ; }
        if(argv[search_index] == "shared_buffer_length"){ shared_buffer_length = hex_string_to_uint32(argv[search_index+1]) ; continue ; }    
    }
